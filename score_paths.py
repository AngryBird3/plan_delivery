...
from nest_info import NestInfo, DeliverySite


def is_keepout_valid(nest_info: NestInfo, path: typing.List["tuple"]) -> bool:
    """
    ...
    Note: This means delivery sites inside of keepouts should always be unreachable
    """
    return all(
        [
            nest_info.risk_zones[int(coord[0])][int(coord[1])] < NestInfo.KEEP_OUT_VALUE
            for coord in path
        ]
    )
...
def is_keepin_valid(nest_info: NestInfo, path: typing.List["tuple"]) -> bool:
    """
    ...
    return all(
        [
            ...
            all([coord[0] <= nest_info.risk_zones.shape[0] for coord in path]),
            all([coord[1] >= 0 for coord in path]),
            all([coord[1] <= nest_info.risk_zones.shape[1] for coord in path]),
        ]
    )
...
def get_results(
    nest_info: NestInfo, delivery_sites_with_paths: typing.List["DeliverySite"]
) -> str:
    results_dict = dict()
    for site in delivery_sites_with_paths:
        errors = []
        if len(site.path) == 0:
        ...
        locations_valid = all(
            [all([isinstance(c, int) for c in coord]) for coord in site.path]
        )
        if not locations_valid:
            errors.append("Not all path elements are integers")

        # check to make sure the path starts and ends at nest and delivery site
        if not site.path[0] == nest_info.nest_coord:
            errors.append("Path does not start at nest coordinates")

        if not site.path[-1] == site.coord:
            errors.append("Path does not end at site coordinates")

        # now check properties of the path itself
        if not is_keepout_valid(nest_info, site.path):
        ...
        if not is_keepin_valid(nest_info, site.path):
            errors.append(
        ...
        if not path_length <= nest_info.maximum_range:
            errors.append(
                "Path length ({}) is above maximum range of the zip ({})".format(
                    path_length, nest_info.maximum_range
                )
            )

        if is_keepin_valid(nest_info, site.path):
            total_risk = sum(
                [
                    nest_info.risk_zones[int(coord[0])][int(coord[1])]
...
def display_and_save_result_image(
    nest_info: NestInfo, delivery_sites: typing.List["DeliverySite"]
) -> None:
    fig = plt.figure()
    ax = fig.add_axes([0.1, 0.1, 0.6, 0.75])
    handles = []

    handles.extend(nest_info.display(ax))

    # Plot each delivery site
    for delivery_site in delivery_sites:
        site_handle = delivery_site.display(ax)
        handles.extend(site_handle)

    # Add titles and labels
    plt.title("Nest {}".format(nest_info.name))
